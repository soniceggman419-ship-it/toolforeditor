...
create_dict = create_int = create_float = create_str = create_bool = create_base_item
...
class Tree(Column):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.item_types = self.compound_types + kwargs.pop('item_types', [a[0] for a in self.map_types_item.values()] or [int, float, str, bool])
...
#        row_height = self.font.size(' ')[1]
    ...
    def add_item_to(self, parent, name_item):
        name, item = name_item
        if parent is None:
            tp = 'dict'
            parent = self.data
        ...
        if item:
            if item[6] in self.deployed:
                ...
                if meth is not None:
                    ...
                    _v = v
                ks = sorted(_v.keys(), reverse=True)
                for a in ks:
                    b = _v[a]
