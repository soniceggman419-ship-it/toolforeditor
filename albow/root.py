...
class RootWidget(Widget):
    ...
    def getKeycode(keyName):
        if not keyName or keyName == "None":
            return None
        lowered = keyName.lower()

        if len(lowered) > 1 and lowered[0] in ('l', 'r') and lowered[1:].isalpha():
            side = "left" if lowered[0] == 'l' else "right"
            try:
                return key.key_code(f"{side} {lowered[1:]}")
            except ValueError:
                pass

        renamed = {"del": "delete", "esc": "escape", "enter": "return", "ins": "insert"}
        mapped = renamed.get(lowered, lowered)

        try:
            return key.key_code(mapped)
        except ValueError:
            pass

        for prefix in ("left ", "right "):
            try:
                return key.key_code(prefix + mapped)
            except ValueError:
                pass
            ...
            while modal_widget.modal_result is None:
                try:
                    ...
                    if self.editor.level is not None:
                        ...
                        if key.get_mods() & (KMOD_LCTRL | KMOD_RCTRL | KMOD_LMETA | KMOD_RMETA):
                            pass
                        else:
                            for keyNum, keyName in enumerate(self.editor.movements):
                                kc = self.getKeycode(keyName)
                                if kc is not None and allKeys[kc]:
                                    self.changeMovementKeys(keyNum, keyName)

                            for keyNum, keyName in enumerate(self.editor.cameraPan):
                                kc = self.getKeycode(keyName)
                                if kc is not None and allKeys[kc]:
                                    self.changeCameraKeys(keyNum)

                        kc = self.getKeycode(self.editor.sprintKey)
                        if kc is not None and allKeys[kc]:
                            self.sprint = True
