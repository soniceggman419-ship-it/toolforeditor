...
BlockRender documentation
  ...
  The makeFaceVertices gets an vertexArray for a particular face.
   ...
   parameters:
    ...
    exposedFaceIndices:
      ...
      in the direction repr(direction).
  ...
  Helper functions:
    ...
    self.makeTemplate(direction, blockIndices):
      get a vertex array filled with default values for face repr(direction)
      and for the block relating to repr(blockIndices)
...
class ChunkCalculator(object):
    ...
    def makeRenderstates(self, materials):
        ...
        self.materialMap = materialMap = numpy.zeros((pymclevel.materials.id_limit,), 'uint16')
    ...
    def calcFacesForChunkRenderer(self, cr):
        ...
        try:
            ...
            if "Session lock lost" in e.message:
                yield
                return
            logging.warning(u"Error reading chunk: %s", e)
            traceback.print_exc()
            yield
            return
