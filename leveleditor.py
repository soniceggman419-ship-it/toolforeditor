    ...
    def addExternalWidget_Dict(self, provided_fields):
        ...
        for inputName, inputType in provided_fields:
            if isinstance(inputType, tuple) and inputType != ():
                if isinstance(inputType[0], (int, int, float)):
                    if len(inputType) == 2:
                        min, max = inputType
                        val = min
                        increment = 0.1
    ...
    def askLoadWorld(self):
        ...
        def dispatch_key(name, evt):
            ...
            if keyname == "Escape":
                ...
                self.worldData.sort(key=lambda row: row[3], reverse=True)
        ...
        def nameFormat(w):
            try:
                ...
                return u"%s" % w.LevelName, u"%s" % w.displayName
            except:
                try:
                    return w.LevelName, w.LevelName
                except:
        ...
        self.worldData.sort(key=lambda row: row[-1], reverse=True)
