...
class CameraViewport(GLViewport):
    ...
    def tickCamera(self, frameStartTime, inputs, inSpace):
        ...
        cameraAccel = tuple(map(lambda x: x * accel_factor * timeDelta, directedInputs))

        newVelocity = tuple(map(lambda a, b: a + b, velocity, cameraAccel))
        ...
        velocity = tuple(map(lambda a: a * speed, velocityDir))

        # velocity = tuple(map(lambda p,d: p + d, velocity, cameraImpulse))
        d = tuple(map(lambda a, b: abs(a - b), self.cameraPosition, self.oldPosition))
        if d[0] + d[2] > 32.0:
            self.oldPosition = self.cameraPosition
            self.updateFloorQuad()

        self.cameraPosition = tuple(map(lambda p, d: p + d * timeDelta, self.cameraPosition, velocity))
    ...
    def _anglesToVector(yaw, pitch):
        ...
        return tuple(map(nanzero, (dx, dy, dz)))
    ...
    def _findBlockFaceUnderCursor(self, projectedPoint):
        ...
        try:
            intProjectedPoint = list(map(int, map(numpy.floor, projectedPoint)))
        except ValueError:
