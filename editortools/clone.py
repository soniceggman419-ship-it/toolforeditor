...
class CloneTool(EditorTool):
    ...
    def rescaleLevel(self, factor):
        ...
        newshape = list(map(lambda x: int(x * factor), oldshape))
    ...
    def _draggingOrigin(self):
        dragPos = list(map(int, map(numpy.floor, self.positionOnDraggingPlane())))
...
#                alert("BO3 support is currently not available")
    ...
    def loadSchematic(self, filename):
        ...
        try:
            ...
            logging.warning(u"Unable to import file %s : %s", filename, e)
