...
class TAG_Value(object):
    ...
    def name(self, newVal):
        ...
        self._name = newVal.decode('utf-8') if isinstance(newVal, bytes) else str(newVal)
...
class TAG_Byte_Array(TAG_Value):
    ...
    def load_from(cls, ctx):
        ...
        value = frombuffer(data[4:string_len * cls.dtype.itemsize + 4], cls.dtype)
        self = cls(value)
        ctx.offset += string_len * cls.dtype.itemsize + 4
        return self

    def write_value(self, buf):
        value_str = self.value.tobytes()
...
def load_string(ctx):
    ...
    raw = data[2:string_len + 2]
    if isinstance(raw, bytes):
        value = raw.decode('utf-8')
    else:
        value = raw.tobytes().decode('utf-8')
