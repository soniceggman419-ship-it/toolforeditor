...
import numpy
import struct
from pymclevel.infiniteworld import ChunkedLevelMixin, SessionLockLost, AnvilChunkData, AnvilWorldFolder, unpackNibbleArray, packNibbleArray
...
class PocketLeveldbDatabase(object):
        ...
        with self.world_db() as db:
            ...
            # Let assume that any chunk wich version is greater or equal to 3 in a PE 1+ one.
                ...
                if d2d:
                    ...
                    chunk.data_2d = d2d
                    biomes = numpy.frombuffer(d2d[512:], 'uint8')
                    biomes.shape = (16, 16)
                    chunk.Biomes = biomes
                for i in range(16):
    ...
    def _saveChunk_1plus(self, chunk, batch=None, writeOptions=None):
        ...
        if hasattr(chunk, 'Biomes') and data_2d:
            data_2d = data_2d[:512] + chunk.Biomes.tobytes()
...
class PocketLeveldbChunkPre1(LightedChunk):
    ...
    def __init__(self, cx, cz, world, data=None, create=False, world_version=None):
        ...
        if create:
            ...
            terrain = numpy.frombuffer(data[0], dtype='uint8')
            if data[1] is not None:
                if DEBUG_PE:
...
#                 print v, id, MCEDIT_DEFS.get(MCEDIT_IDS.get(v, v), {'id': -1})['id']
        ...
        terrain = ''.join([self.Blocks.tobytes(),
                           packData(self.Data).tobytes(),
                           packData(self.SkyLight).tobytes(),
                           packData(self.BlockLight).tobytes(),
                           self.DirtyColumns.tobytes(),
                           self.GrassColors.tobytes(),
                           ])
...
# =====================================================================
class PE1PlusDataContainer(object):
    ...
    def add_data(self, terrain=None, tile_entities=None, entities=None, subchunk=None):
        ...
        if terrain:
            ...
            if subchunk_version in [0, 2, 3, 4, 5, 6, 7]:
                ...
                self._Blocks.add_data(subchunk, numpy.frombuffer(blocks, "uint8").astype(self._Blocks.bin_type))

                #             for k, v in ((self._Data, data), (self._SkyLight, skyLight), (self._BlockLight, blockLight)):
                #                 a = numpy.frombuffer(v, k.bin_type)
                #                 a.shape = (16, 16, len(v) / 256)
                #                 k.add_data(subchunk, unpackNibbleArray(a).tobytes())

                a = numpy.frombuffer(data, "uint8")
                a.shape = (16, 16, len(data) / 256)
                self._Data.add_data(subchunk, numpy.frombuffer(unpackNibbleArray(a).tobytes(), "uint8").astype(self._Data.bin_type))

                if self.chunk_version == "\x03":
                    for k, v in ((self._SkyLight, skyLight), (self._BlockLight, blockLight)):
                        a = numpy.frombuffer(v, "uint8")
                        a.shape = (16, 16, len(v) / 256)
                        k.add_data(subchunk,
                                   numpy.frombuffer(unpackNibbleArray(a).tobytes(), "uint8").astype(k.bin_type))
