...
from cpython cimport PyTypeObject, PyUnicode_DecodeUTF8, PyList_Append, PyBytes_FromStringAndSize
...
IF UNICODE_NAMES:
    cdef unicode load_name(load_ctx ctx):
        ...
        b = read(ctx, length)
        IF UNICODE_CACHE:
            s = PyBytes_FromStringAndSize(b, length)
            u = u_cache.get(s)
            if u is None:
...
cdef TAG_Byte_Array load_byte_array(load_ctx ctx):
    ...
    return TAG_Byte_Array(numpy.frombuffer(arr[:byte_length], dtype=TAG_Byte_Array.dtype, count=length))
...
cdef TAG_Short_Array load_short_array(load_ctx ctx):
    ...
    return TAG_Short_Array(numpy.frombuffer(arr[:byte_length], dtype=numpy.dtype(dtype), count=length))
...
cdef TAG_Int_Array load_int_array(load_ctx ctx):
    ...
    return TAG_Int_Array(numpy.frombuffer(arr[:byte_length], dtype=numpy.dtype(dtype), count=length))
...
cdef TAG_Long_Array load_long_array(load_ctx ctx):
    ...
    return TAG_Long_Array(numpy.frombuffer(arr[:byte_length], dtype=numpy.dtype(dtype), count=length))
...
def hexdump(src, length=8):
    FILTER=bytes([(x if 32 <= x < 127 else ord('.')) for x in range(256)])
    N=0
    result=''
    while src:
