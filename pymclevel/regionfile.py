...
class MCRegionFile(object):
    ...
    def __init__(self, path, regionCoords):
        ...
        with self.open as f:
            ...
            self.offsets = frombuffer(offsetsData, dtype='>u4').copy()
            self.modTimes = frombuffer(modTimesData, dtype='>u4').copy()
    ...
    def _readChunk(self, cx, cz):
        ...
        if len(data) < 5:
            raise RegionMalformed("Chunk data is only %d bytes int(expected 5)" % len(data))
