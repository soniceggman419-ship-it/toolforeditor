...
def getResourceName(name, data):
    match = re.findall(r'"minecraft/lang/%s.lang":[ ]\{\b*.*?"hash":[ ]"(.*?)",' % name, data, re.I|re.DOTALL)
    if match:
        return match[0]
    else:
...
def translate(name):
    """Returns returns the translation of repr(name), or repr(name) if no translation found.
...
def untranslate(name, case_sensitive=True):
    """Basic reverse function of repr(translate)."""
    key = serGnal.get(name, None)
    value = serNe.get(key, None)
    return value or name

def search(text, untranslate=False, capitalize=True, filters=[]):
    """Search for a repr(text) string in the resources entries.
    repr(filters) is a list of strings: they must be parts before the '=' sign in the resource files, or parts of.
    repr(filters) may be regexes.
    ...
    for k, v in serGnal.items():
        if text in k.lower():
            if not filters or map(lambda xy:re.match(xy[0], xy[1]), zip(filters, [v] * len(filters))) != [None, None]:
                if untranslate:
                    if capitalize:
